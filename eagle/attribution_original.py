...
class ModalityAttributionAnalyzer:
    ...
    def _register_hooks(self):
        ...
        if hasattr(self.model, "imaging_text_attention"):
            ...
            if hasattr(self.model, "imaging_clinical_attention"):
                ...
                self.model.imaging_clinical_attention.register_forward_hook(
                    clinical_attention_hook
                )
    ...
    def compute_gradient_attribution(self, sample_idx: int) -> Dict[str, float]:
        ...
        # Prepare inputs
        imaging = (
            sample["imaging_features"].unsqueeze(0).to(self.device).requires_grad_(True)
        )
        text_emb = {
        ...
        text_feat = (
            sample["text_features"].unsqueeze(0).to(self.device).requires_grad_(True)
        )
    ...
    def compute_integrated_gradients(
        self, sample_idx: int, n_steps: int = 50
    ) -> Dict[str, float]:
        ...
        for step in range(n_steps):
            ...
            text_emb_interp = {}
            for k in text_emb:
                text_emb_interp[k] = text_emb_baseline[k] + alpha * (
                    text_emb[k] - text_emb_baseline[k]
                )
            ...
            text_feat_interp = text_feat_baseline + alpha * (
                text_feat - text_feat_baseline
            )
            text_feat_interp.requires_grad_(True)

            # Forward pass
            risk_score, _ = self.model(
                imaging_interp, text_emb_interp, clinical_interp, text_feat_interp
            )
        ...
        text_ig = sum(
            torch.stack([g[i] for g in text_grads]).mean(0)
            * (text_emb[k] - text_emb_baseline[k])
            for i, k in enumerate(text_emb)
        )
        clinical_ig = torch.stack(clinical_grads).mean(0) * (
    ...
    def analyze_patient(self, sample_idx: int) -> AttributionResult:
        ...
        for modality in ["imaging", "text", "clinical"]:
            modality_scores[modality] = (
                gradient_attrs[modality] + integrated_grads[modality]
            ) / 2
        ...
        if self.attention_weights:
            attention_weights = {
                k: v.cpu().numpy() for k, v in self.attention_weights.items()
            }
...
def plot_patient_level_attribution(
    ...
    ax.set_ylabel("Contribution (%)", fontsize=12)
    ax.set_title(
        f"Modality Contributions for Patient {result.patient_id}\nRisk Score: {result.risk_score:.3f}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    if result.feature_level_attributions:
        ...
        if top_features:
            ...
            ax.text(
                0.5,
                0.5,
                "Feature-level attribution not available",
                ha="center",
                va="center",
