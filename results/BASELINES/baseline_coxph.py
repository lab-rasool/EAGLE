"""
Cox Proportional Hazards baseline for EAGLE comparison
"""
...
def train_coxph(X_train, y_time_train, y_event_train, penalizer=0.1, l1_ratio=0):
    ...
    # Fit Cox model (without step_size parameter)
...
def main():
    parser = argparse.ArgumentParser(description="CoxPH baseline for survival prediction")
    ...
    log_file = Path(args.output_dir) / f"coxph_{args.dataset}_{timestamp}.log"
    ...
    logging.info(f"Starting CoxPH baseline for {args.dataset}")
    ...
    # Convert to contiguous arrays to avoid index issues
    ...
    # Create a new array of indices for cross-validation
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(indices, y_event)):
        ...
        try:
            ...
            logging.warning(f"Fold {fold + 1} failed with penalizer={args.penalizer}: {str(e)}")
            # Try with more regularization
            increased_penalizer = args.penalizer * 10
            try:
                model = train_coxph(X_train, y_time_train, y_event_train, 
                                  penalizer=increased_penalizer, l1_ratio=args.l1_ratio)
                c_index, _ = evaluate_coxph(model, X_val, y_time_val, y_event_val)
                cv_scores.append(c_index)
                successful_folds += 1
                logging.info(f"Fold {fold + 1} C-index (penalizer={increased_penalizer}): {c_index:.4f}")
            except Exception as e2:
                # Try with even more regularization and Elastic Net
                try:
                    model = train_coxph(X_train, y_time_train, y_event_train, 
                                      penalizer=args.penalizer * 100, l1_ratio=0.5)
                    c_index, _ = evaluate_coxph(model, X_val, y_time_val, y_event_val)
                    cv_scores.append(c_index)
                    successful_folds += 1
                    logging.info(f"Fold {fold + 1} C-index (high regularization): {c_index:.4f}")
                except Exception as e3:
                    logging.error(f"Fold {fold + 1} failed completely: {str(e3)}")
    ...
    # Save results
    results_file = Path(args.output_dir) / f"coxph_{args.dataset}_{timestamp}_results.json"
